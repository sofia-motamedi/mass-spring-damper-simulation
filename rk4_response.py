...
# Solve the system using the 4th Order Runge-Kutta Method:
for i in range(len(t) - 1):
    # Compute s1 values:
    s1_x = dt * v[i]
    s1_v = dt * (np.sin(2 * np.pi * t[i]) - c * v[i] - k * x[i])

    # Compute s2 values using s1:
    s2_x = dt * (v[i] + 0.5 * s1_v)
    s2_v = dt * (np.sin(2 * np.pi * (t[i] + 0.5 * dt)) - c * (v[i] + 0.5 * s1_v) - k * (x[i] + 0.5 * s1_x))

    # Compute s3 values using s2:
    s3_x = dt * (v[i] + 0.5 * s2_v)
    s3_v = dt * (np.sin(2 * np.pi * (t[i] + 0.5 * dt)) - c * (v[i] + 0.5 * s2_v) - k * (x[i] + 0.5 * s2_x))

    # Compute s4 values using s3:
    s4_x = dt * (v[i] + s3_v)
    s4_v = dt * (np.sin(2 * np.pi * (t[i] + dt)) - c * (v[i] + s3_v) - k * (x[i] + s3_x))

    # Update the state
    x[i+1] = x[i] + (s1_x + 2*s2_x + 2*s3_x + s4_x) / 6
    v[i+1] = v[i] + (s1_v + 2*s2_v + 2*s3_v + s4_v) / 6
...
plt.title("Mass-Spring-Damper System Response (RK4 Method)")
plt.grid(True)
plt.legend()
